...
export default function Dash({ P }) {
  // Use a single loop pattern to process all these arrays instead of N repeated filters
  const { m, distribution } = useMemo(() => {
    ...
    return {
      m: { openT, inProgress, resolved, dueMaint, overdueM, openInc, onDuty, totalCost, avgTAT, lowStock, expiringAMC, overdueA, whatsapp, priority, criticalCount: criticalT.length },
      distribution: distr
    };
