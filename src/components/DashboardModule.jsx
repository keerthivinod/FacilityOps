...
export default function Dash({ P }) {
  // ⚡ Bolt Optimization: Replace multiple O(N) array filter/reduce operations with a single O(N) pass loop.
  // This memoized block prevents massive re-computations when dashboard renders.
  const m = useMemo(() => {
    ...
    for (let i = 0; i < P.tickets.length; i++) {
      ...
      if (stat === "open") openT++;
      else if (stat === "in-progress") inProgress++;
      else if (stat === "resolved" || stat === "closed") resolved++;

      if (stat !== "resolved" && stat !== "closed") {
        if (prio === "critical") criticalT.push(t);
        else if (prio === "high") highT.push(t);
      }

      if (t.cost) totalCost += t.cost;
      if (t.tatMins) { tatSum += t.tatMins; tatCount++; }
      if (t.source === "whatsapp") whatsapp++;

      if (prio === "critical") critCount++;
      else if (prio === "high") highCount++;
      else if (prio === "medium") medCount++;
      else if (prio === "low") lowCount++;
    }
    ...
    for (let i = 0; i < P.tasks.length; i++) {
      const stat = P.tasks[i].status;
      if (stat === "overdue" || stat === "due-soon") dueMaint++;
      if (stat === "overdue") overdueM++;
    }
    ...
    const distTotal = P.tickets.length;
    const distribution = [
      { p: "critical", cnt: critCount, pct: distTotal ? Math.round((critCount / distTotal) * 100) : 0 },
      { p: "high",     cnt: highCount, pct: distTotal ? Math.round((highCount / distTotal) * 100) : 0 },
      { p: "medium",   cnt: medCount,  pct: distTotal ? Math.round((medCount / distTotal) * 100) : 0 },
      { p: "low",      cnt: lowCount,  pct: distTotal ? Math.round((lowCount / distTotal) * 100) : 0 }
    ];

    return {
      openT, inProgress, resolved, dueMaint, overdueM, openInc, onDuty,
      totalCost, avgTAT, lowStock, expiringAMC, overdueA, whatsapp,
      priority, criticalCount: criticalT.length, distribution
    };
